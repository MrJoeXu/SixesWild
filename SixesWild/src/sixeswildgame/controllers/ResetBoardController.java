...
public class ResetBoardController implements ActionListener {
	...
	/**
	 * Creates new ResetBoardController with specified application and level
	 * 
	 * @param application
	 * @param level
	 */
	...
	/**
	 * Resets the board when playing the level
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		if (level.getResetBoardMoves() > 0) {
			...
			for (Space sp : level.getBoard().getGrid()) {
				if (sp.isEnabled() && sp.getTile().getValue() != 6
						&& sp.getTile().getValue() != 7) {
					spaceGrid.add(sp);
					indices.add(sp.getTile().getRow()
							* level.getBoard().getDimension()
							+ sp.getTile().getColumn());
				}
			}
			...
			for (int l = 0; l < level.getBoard().getDimension()
					* level.getBoard().getDimension(); l++) {
				if (level.getBoard().getGrid().get(l).getTile().getValue() != 6
						&& level.getBoard().getGrid().get(l).getTile()
								.getValue() != 7
					...
					level.getBoard()
							...
							.get(l)
							.setTile(
									new Tile(spaceGrid.get(k).getTile()
											.getValue(), spaceGrid.get(k)
											.getTile().getBonus(), i, j,
											spaceGrid.get(k).getTile()
													.getLastValue(), spaceGrid
													.get(k).getTile()
													.getLastBonus(), spaceGrid
													.get(k).getTile()
													.isSelected()));
					k++;
				}
				...
				System.out.println("Row: "
						+ level.getBoard().getGrid().get(k).getTile().getRow()
						+ ", Column: "
						+ level.getBoard().getGrid().get(k).getTile()
								.getColumn());
			}
			...
			application.getLevelView().getResetBoardCountLbl()
					.setText(String.valueOf(level.getResetBoardMoves()));

			for (SpaceView sv : application.getLevelView().getBoardView()
					.getGrid()) {
				TileView tv = sv.getTileView();
				tv.addMouseListener(new MakeMoveController(tv, level,
						application));
